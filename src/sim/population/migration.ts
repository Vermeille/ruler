...
export const migrationRule: Rule = {
  ...
  run({ model, cache, random }) {
    return planMigration(model, cache, random).map(move => ({
...
export const migrationCashRule: Rule = {
  ...
  run({ model, cache, random }) {
    ...
    for (const move of planMigration(model, cache, random)) {
    ...
    return [...byRoute.values()].map(route => ({
      ...
      amount: route.amount
        * Math.max(0, model.cells[route.from].cash)
        / Math.max(1e-12, cache.peopleByCell[route.from].population),
    }));
  },
};
