...
The executable source of truth for field semantics is `MAPXEL_FIELDS` in `src/sim/map-fields.ts` and `SUMMARY_FIELDS` in `src/sim/math.ts`. Every exposed numeric mapxel/summary field has a machine-readable unit and description, so tests, reports, and developer tooling can present effect sizes dimensionally rather than comparing unexplained raw numbers. A unit of `index` is explicitly dimensionless and normalized; `share` is a dimensionless fraction; `price-index` is relative to reference price 1. `activity/month` remains an abstract economic-activity unit and must not be displayed or interpreted as crowns.

`foodSecurity` is not an independent food stock. During consumption it is the realized ratio `min(1, food consumed / population)`: one person-month of food is required per resident per month. Because production and trade precede consumption, this ratio measures whether the month's available food after those corrective mechanisms actually met household need.
