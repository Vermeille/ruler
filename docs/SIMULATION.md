...
Each land cell has terrain, fertility, mineral suitability, private cash, food and material stocks, industry shares, demographic fractions, health, education, happiness, approval, employment, crime pressure, pollution, transport quality, business viability, and sports interest. Indices lie in [0,1]; food prices lie in [0.4,5]. Demographic fractions leave an implicit working-age share. A country has four named regions and a finite external cash account in addition to its treasury and debt.
...
| Market | Food prices respond gradually to scarcity; shortages and ingredient prices weaken restaurants/shops |
...
| Fiscal | Interest is paid first; remaining funds pay for services and subsidies proportionately |
| Society | Public services, wealth, crime, food, employment, and liberties gradually change living conditions; expected births/deaths change population |
| Migration | Small numbers move across neighboring borders toward opportunity, carrying proportional savings |
...
- Higher reserves increase imports, creating a savings equilibrium. Neighbor migration is capped at 0.3% per edge per month.
...
The regression suite checks five 48-month seeds, 240 months without discrete events, and a tiny perturbation of an initial condition. It checks complete account conservation, policy directionality, a subsidy/food/business chain, and deliberately unaffordable fiscal settings. `npm run calibrate` additionally runs five scenarios on three full-size 36×26 worlds. The baseline's observed 48-month outcomes are 73.1–73.7% approval, 80.2–81.1% wellbeing, 99.7–99.8% food needs met, no debt, and full service funding. National sports subsidies of ₡3 produce temporary food coverage lows around 64%, then recovery as farming becomes attractive and funding is constrained. These thresholds describe this model, not a proof that every future rule combination is stable.
