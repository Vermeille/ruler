...
function reportMigrationAmounts(): void {
  const effects = migrationRule.run({ model: game.model, random: () => 0.5, lastEvents: game.lastEvents });
  ...
  console.log('MIGRATION_AMOUNTS', JSON.stringify({
    ...
    under001: under(.01), under005: under(.05), under01: under(.1), under025: under(.25), under05: under(.5),
  }));
}
...
for (let month = 1; month <= 12; month += 1) {
  ...
  if (month % 3 === 0) reportMigrationAmounts();
  if (month === 11) {
